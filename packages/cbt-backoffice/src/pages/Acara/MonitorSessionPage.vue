                ...
                <td class="px-4 py-3">
                  <div class="flex flex-wrap items-center justify-center gap-2">
                    ...
                    <Button
                      ...
                      @click="router.push(`/acara/${route.params.id}/monitor/${jadwalId}/${session.id}`)"
                      variant="outline"
                      size="sm"
                      class="gap-1.5"
...
const stats = computed(() => {
  return {
    ...
    completed: sessions.value.filter((s: IProktorSession) => s.status === 'finished').length,
    inProgress: sessions.value.filter((s: IProktorSession) => s.status === 'in_progress').length,
    notStarted: sessions.value.filter((s: IProktorSession) => s.status === 'not_started').length
  }
})
...
const filteredSessions = computed(() => {
  ...
  if (filterStatus.value !== 'all') {
    result = result.filter((s: IProktorSession) => s.status === filterStatus.value)
  }

  if (searchQuery.value.trim()) {
    const query = searchQuery.value.toLowerCase()
    result = result.filter((s: IProktorSession) =>

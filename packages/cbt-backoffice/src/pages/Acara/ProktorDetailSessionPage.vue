...
const correctCount = computed(() =>
  results.value?.questions.filter((q: ResultQuestion) => q.isAnswered && q.options.some((o: { isSelected: boolean; isCorrect: boolean }) => o.isSelected && o.isCorrect)).length ?? 0
)
const wrongCount = computed(() =>
  results.value?.questions.filter((q: ResultQuestion) => q.isAnswered && !q.options.some((o: { isSelected: boolean; isCorrect: boolean }) => o.isSelected && o.isCorrect)).length ?? 0
)
const unansweredCount = computed(() =>
  results.value?.questions.filter((q: ResultQuestion) => !q.isAnswered).length ?? 0
)
